...
execute positioned ~-.15 ~-.15 ~-.15 as @a[dx=0,predicate=!game:de/player_entity] positioned ~-.7 ~-.7 ~-.7 if entity @s[dx=0] run tag @s add flame_hit
...
data modify entity @e[type=small_fireball,tag=new,limit=1] Owner set from entity @a[predicate=game:de/player_entity,limit=1] UUID
execute if entity @a[tag=flame_hit,limit=1] run kill @s

scoreboard players remove @a[tag=flame_hit,limit=1] player.health 1
