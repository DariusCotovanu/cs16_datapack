...
execute store result entity @s Rotation[0] float 0.1 run scoreboard players get item_rotation cs16.temp
...
execute store result score $gun_id cs16.temp run data get entity @s item.components.minecraft:custom_data.gun.id
execute if score $gun_id cs16.temp matches 30 run tag @s add awp
...
execute if score $gun_id cs16.temp matches 99 run tag @s add grenade